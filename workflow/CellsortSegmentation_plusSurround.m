function [ica_segments, segmentlabel, segcentroid, maj_axis, ICused] = CellsortSegmentation_plus(ica_filters, smwidth, thresh, arealims, plotting, surround_radius_addition)
...
% Adding this in here so that output is always assigned
maj_axis = [];
...
for j = 1:nic
    ...
    Lcent = regionprops(L, 'Centroid','MajorAxisLength');
  ...
  % inserting the code below to pull out major axis length
    for jj = 1:length(Lbig)
        maj_axis(jj+k) = Lcent(Lbig(jj)).MajorAxisLength;
    end
    ...
    for jj = 1:length(Lbig)
        ICused = [ICused j];
        surrL = L;
        current_center = round(segcentroid(jj+k,:));
        cy = current_center(1); cx = current_center(2);
        radius = round(maj_axis(jj+k)/2) + surround_radius_addition;
        
        for x = abs(cx-2*radius) : cx +2*radius
            if x > size(L,1)
                x = x+1;
            else
                for y = abs(cy-2*radius) : cy +2*radius
                    if y > size(L,2)
                        y = y+1;
                    else
        ...
        Lprocessed = ( 0*(L==0) + (L==Lbig(jj)) );
        ica_segments(jj+k,:,:) = ica_filtersuse .* Lprocessed ;  % Exclude background
...
% % I am inserting here a figure command to try and keep all of the independent components..
         figure
        if smwidth>0
            subplot(2,2,2)
